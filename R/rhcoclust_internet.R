...
for (i in 1:length(ColClust)) {
  #SortIndx<-sort(1:ncol(CoClustObj$CoClsDtMat),decreasing = T)
  #CoClustObj$CoClsDtMat1 <- CoClustObj$CoClsDtMat[SortIndx,SortIndx]
  ColCoClust[[i]] <- sapply(len.1, function(len.1) paste0(ColClust[i],RowClust[len.1]))
}
...
for (ii in 1:ncol(resultRow)) {
  CombRow[[ii]] <- combn(resultRow[, ii], 2)[, 1:length(CoClustObj$rowclust)]
}
# graph for row
...
for (jj in 1:ncol(resultCol)) {
  CombCol[[jj]] <- combn(resultCol[, jj], 2)[, 1:length(CoClustObj$colclust)]
}
# graph for col
...
for (k in 1:length(CoClustObj$rowclust)) {
  data_weight[[k]] <- sapply(len_rowclust, function(len_rowclust) CoClustObj$CoClsDtMat[match(CoClustObj$rowclust[[k]],rownames(CoClustObj$CoClsDtMat)),match(CoClustObj$colclust[[len_rowclust]],colnames(CoClustObj$CoClsDtMat))])
}

# mean for genes (rows)
mean_rows <- list()
for (kk in 1:length(CoClustObj$rowclust)) {
  mean_rows[[kk]] <- sapply(len_rowclust, function(len_rowclust) apply(data_weight[[kk]][[len_rowclust]], 1, mean)) # mean for genes (rows)
}
# rbind rows
